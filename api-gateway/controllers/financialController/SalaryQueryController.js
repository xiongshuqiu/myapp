...
// 1. 获取所有床位分配
const getAllSalaryQueries = async (req, res) => {
...
const renderNewSalaryQueryForm = async (req, res) => {
  try {
...
// (2) 提交新的床位分配数据
const createSalaryQuery = async (req, res) => {
  const {
    availableBedId,
    unassignedElderlyId,
    assignmentId,
...
// 3. 更新特定床位分配
// (1) 查找特定床位分配并显示编辑表单
const getSalaryQueryById = async (req, res) => {
  try {
...
// (2) 提交更新后的床位分配数据
const updateSalaryQuery = async (req, res) => {
...
// 4. 删除特定床位分配
const deleteSalaryQuery = async (req, res) => {
  try {
