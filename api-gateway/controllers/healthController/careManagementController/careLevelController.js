...
const getAllCareLevels = async (req, res) => {
  ...
  try {
    const url = `${process.env.HEALTH_SERVICE_URL}/health/care/level/?_id=${_id}&role=${role}`;
...
const renderNewCareLevelForm = async (req, res) => {
  try {
    const url = `${process.env.HEALTH_SERVICE_URL}/health/care/level/new`;
    const response = await getRequest(url); // 发送 GET 请求以获取用户信息
    res.json(response); // 将响应数据返回给前端:包括数据和message
    if (response.success) {
      console.log(response);
...
const createCareLevel = async (req, res) => {
  ...
  try {
    ...
    const url = `${process.env.HEALTH_SERVICE_URL}/health/care/level/create`;
...
const getCareLevelById = async (req, res) => {
  try {
    ...
    const url = `${process.env.HEALTH_SERVICE_URL}/health/care/level/${_id}/update`;
    const response = await getRequest(url); // 发送 GET 请求以获取用户信息
    res.json(response); // 将响应数据返回给前端:包括数据和message
    if (response.success) {
      console.log(response);
...
const updateCareLevel = async (req, res) => {
  ...
  try {
    ...
    const { _id } = req.params; // 从参数中获取 _Id
    const url = `${process.env.HEALTH_SERVICE_URL}/health/care/level/${_id}`;
...
const deleteCareLevel = async (req, res) => {
  try {
    ...
    const url = `${process.env.HEALTH_SERVICE_URL}/health/care/level/${_id}/delete`;
