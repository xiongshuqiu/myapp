...
const getAllCarePlans = async (req, res) => {
  ...
  try {
    const url = `${process.env.HEALTH_SERVICE_URL}/health/care/plan/?_id=${_id}&role=${role}`;
...
const renderNewCarePlanForm = async (req, res) => {
  try {
    const url = `${process.env.HEALTH_SERVICE_URL}/health/care/plan/new`;
    const response = await getRequest(url); // 发送 GET 请求以获取用户信息
    res.json(response); // 将响应数据返回给前端:包括数据和message
    if (response.success) {
      console.log(response);
...
const createCarePlan = async (req, res) => {
  ...
  try {
    ...
    const url = `${process.env.HEALTH_SERVICE_URL}/health/care/plan/create`;
...
const getCarePlanById = async (req, res) => {
  try {
    ...
    const url = `${process.env.HEALTH_SERVICE_URL}/health/care/plan/${_id}/update`;
    const response = await getRequest(url); // 发送 GET 请求以获取用户信息
    res.json(response); // 将响应数据返回给前端:包括数据和message
    if (response.success) {
      console.log(response);
...
const updateCarePlan = async (req, res) => {
  ...
  try {
    ...
    const { _id } = req.params; // 从参数中获取 _Id
    const url = `${process.env.HEALTH_SERVICE_URL}/health/care/plan/${_id}`;
...
const deleteCarePlan = async (req, res) => {
  try {
    ...
    const url = `${process.env.HEALTH_SERVICE_URL}/health/care/plan/${_id}/delete`;
