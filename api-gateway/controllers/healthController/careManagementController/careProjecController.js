...
const getAllCareProjects = async (req, res) => {
  ...
  try {
    const url = `${process.env.HEALTH_SERVICE_URL}/health/care/project/?_id=${_id}&role=${role}`;
...
const renderNewCareProjectForm = async (req, res) => {
  try {
    const url = `${process.env.HEALTH_SERVICE_URL}/health/care/project/new`;
    const response = await getRequest(url); // 发送 GET 请求以获取用户信息
    res.json(response); // 将响应数据返回给前端:包括数据和message
    if (response.success) {
      console.log(response);
...
const createCareProject = async (req, res) => {
  ...
  try {
    ...
    const url = `${process.env.HEALTH_SERVICE_URL}/health/care/project/create`;
...
const getCareProjectById = async (req, res) => {
  try {
    ...
    const url = `${process.env.HEALTH_SERVICE_URL}/health/care/project/${_id}/update`;
    const response = await getRequest(url); // 发送 GET 请求以获取用户信息
    res.json(response); // 将响应数据返回给前端:包括数据和message
    if (response.success) {
      console.log(response);
...
const updateCareProject = async (req, res) => {
  ...
  try {
    ...
    const { _id } = req.params; // 从参数中获取 _Id
    const url = `${process.env.HEALTH_SERVICE_URL}/health/care/project/${_id}`;
...
const deleteCareProject = async (req, res) => {
  try {
    ...
    const url = `${process.env.HEALTH_SERVICE_URL}/health/care/project/${_id}/delete`;
