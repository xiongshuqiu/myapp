...
const handleError = (
  ...
  res,
  targetPage = 'health/careManagement/careLevelCreate',
  msg = 'Server error',
) => {
...
const getAllCareLevels = async (req, res) => {
  ...
  const apiUrl = `${process.env.API_URL}/api/health/care/level/?_id=${_id}&role=${role}`;
  ...
  try {
    ...
    if (response.success) {
      res.render('health/careManagement/careLevelManagement', {
        activePage: 'elderly-management',
        elderlyRecords,
        navItems: req.navItems, // 将导航项传递到视图
...
const renderNewCareLevelForm = async (req, res) => {
  const apiUrl = `${process.env.API_URL}/api/health/care/level/new`;
  console.log('API URL:', apiUrl); // 调试信息
  try {
    const response = await getRequest(apiUrl);
    ...
    if (response.success) {
      ...
      res.render('health/careManagement/careLevelCreate.ejs', {
        activePage: 'elderly-management',
        navItems: req.navItems, // 将导航项传递到视图
...
const createCareLevel = async (req, res) => {
  ...
  try {
    ...
    const apiUrl = `${process.env.API_URL}/api/health/care/level/create`;
    const response = await postRequest(apiUrl, data);
    const elderlyRecord = response.data;
    console.log(elderlyRecord);
    if (response.success) {
      console.log(response);
      res.redirect('/health/care/level/');
    }
  } catch (err) {
    const targetPage = 'health/careManagement/careLevelCreate'; //用户需要输入新值
...
const getCareLevelById = async (req, res) => {
  ...
  try {
    const apiUrl = `${process.env.API_URL}/api/health/care/level/${_id}/update`;
    console.log(apiUrl);
    const response = await getRequest(apiUrl); // 使用组装的URL进行API调用
    const { elderlyRecord, userIds, employeeIds } = response.data;
    if (response.success) {
      console.log(response);
      res.render('health/careManagement/careLevelUpdate.ejs', {
        activePage: 'elderly-management',
...
const updateCareLevel = async (req, res) => {
  ...
  try {
    ...
    // 从请求参数中获取 _id
    const apiUrl = `${process.env.API_URL}/api/health/care/level/${_id}`;
    ...
    const targetPage = 'health/careManagement/careLevelUpdate'; //用户需要输入新值
    handleError(err, req, res, targetPage);
  }
};
...
const deleteCareLevel = async (req, res) => {
  try {
    ...
    console.log(_id);
    const apiUrl = `${process.env.API_URL}/api/health/care/level/${_id}/delete`;
