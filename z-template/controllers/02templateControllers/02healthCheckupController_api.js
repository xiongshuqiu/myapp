...
const getAllHealthCheckups = async (req, res) => {
  ...
  try {
    const url = `${process.env.HEALTH_SERVICE_URL}/health/checkup/?_id=${_id}&role=${role}`;
...
const getHealthCheckupById = async (req, res) => {
  try {
    ...
    const url = `${process.env.HEALTH_SERVICE_URL}/health/checkup/${_id}/update`;
    const response = await getRequest(url); // 发送 GET 请求以获取用户信息
    res.json(response); // 将响应数据返回给前端:包括数据和message
    if (response.success) {
      console.log(response);
...
const deleteHealthCheckup = async (req, res) => {
  try {
    ...
    const url = `${process.env.HEALTH_SERVICE_URL}/health/checkup/${_id}/delete`;
